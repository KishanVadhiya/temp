...
export class TilesManager {
    /** @type {Tile[][]} A 2D array representing the currently visible `Tile` instances. */
    readonly visibleTiles: Tile[][];

    /** @type {HTMLDivElement[]} An array of `HTMLDivElement`s, where each div acts as a container for a row of visible tiles. */
    private visibleTilesRowDivArr: HTMLDivElement[];

    /** @type {MultipleSelectionCoordinates} Stores the coordinates of the currently selected cell range, used for highlighting. */
    private selectionCoordinates: MultipleSelectionCoordinates;

    /** @type {number[][]} A 2D array holding row-wise prefix sums of visible row heights. This helps in calculating vertical positions. */
    private visibleTilesRowPrefixSum: number[][];

    /** @type {number[][]} A 2D array holding column-wise prefix sums of visible column widths. This helps in calculating horizontal positions. */
    private visibleTilesColumnPrefixSum: number[][];

    /** @type {number} The count of visible tile rows. */
    private visibleRowCnt: number;
    /** @type {number} The count of visible tile columns. */
    private visibleColumnCnt: number;

    /** @type {number} The global row index of the first visible tile row. */
    private startRowIdx: number;
    /** @type {number} The global column index of the first visible tile column. */
    ...
    /** @type {CellsManager} An instance of `CellsManager` responsible for managing cell data and interactions. */
     ...
     */
    scrollLeft(): void {
        if (this.startColIdx === 0) return; // Prevent scrolling beyond the first column
        this.gridDiv.style.marginLeft = `${this.marginLeft.value}px`; // Apply new left margin
    ...
    private initialLoad(): void {
        ...
        for (let i = this.startRowIdx; i < this.visibleRowCnt + this.startRowIdx; i++) {
            ...
            const currentVisibleRow: Tile[] = []; // Array to hold Tile objects for the current row
    ...
    private mountTileBottom(): void {
        ...
        const currentVisibleRow: Tile[] = []; // Array to hold new Tile objects
        ...
        for (let j = 0; j < this.visibleColumnCnt; j++) {
            ...
            const tile = new Tile(
                rowIdx,
                colIdx,
                this.visibleTilesRowPrefixSum[this.visibleTilesRowPrefixSum.length - 1], // Use last row's prefix sum
    ...
    private mountTileTop(): void {
        ...
        for (let j = 0; j < this.visibleColumnCnt; j++) {
            ...
            const tile = new Tile(
                rowIdx,
                colIdx,
                this.visibleTilesRowPrefixSum[0], // Use first row's prefix sum
                this.visibleTilesColumnPrefixSum[j], // Use corresponding column's prefix sum
                this.selectionCoordinates,
                this.CellsManager
            );
    ...
    private mountTileLeft(): void {
        ...
        for (let i = 0; i < this.visibleRowCnt; i++) {
            ...
            const tile = new Tile(
                rowIdx,
                colIdx,
                this.visibleTilesRowPrefixSum[i], // Use corresponding row's prefix sum
